<script>
    ...
    import { calculateFileHash, cropImageToSquare } from '$lib/utils';
    ...
    const handleFileSelect = async (event) => {
        ...
        try {
            ...
            uploadApi
                ...
                .catch((error) => {
                    if (network.state.isOnline) {
                        errorMessage = error.message;
                        throw new Error(error.message);
                    } else {
                        errorMessage = "Warning: You're offline. Files you upload now will only exist in your browser cache";
            ...
            // MAYBE MOVE THIS SAVE AND UPDATE INTO PROJECT
            // Save the file itself to IDB
            await db.saveTexture(fileHash, croppedFile, {
                fileName,
                projectId,
                fileHash
            });
