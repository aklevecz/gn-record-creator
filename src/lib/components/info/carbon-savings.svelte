<script>
    ...
    $effect(() => {
        ...
        // Change text color based on normalized value
        if (normalizedValue < 0.3) {
            textColor = 'white'; // Low savings
        } else if (normalizedValue < 0.7) {
            textColor = '#FFEB3B'; // Medium savings (yellow)
        } else {
...
<span style="color: {textColor}" class="font-semibold"
    >CO₂ Savings
...
<div class="relative w-[120px]">
    ...
    >
        ...
        <line x1="12" y1="20" x2="12" y2={20 - tween.current} stroke="#4CAF50" stroke-width="1.5"></line>

        <!-- Left leaf -->
        {#if leftLeafTween.current > 0}
            <path
                d="M12,{20 - tween.current * 0.6} C10,{18 - tween.current * 0.6} 8,{19 - tween.current * 0.6} 9,{16 - tween.current * 0.6}"
                stroke="#66BB6A"
                stroke-width="1"
                fill="#81C784"
                style="transform-origin: 12px {20 - tween.current * 0.6}px; transform: scale({leftLeafTween.current / 4});"
            ></path>
        {/if}
        ...
        {#if rightLeafTween.current > 0}
            <path
                d="M12,{20 - tween.current * 0.4} C14,{18 - tween.current * 0.4} 16,{19 - tween.current * 0.4} 15,{16 - tween.current * 0.4}"
                stroke="#66BB6A"
                stroke-width="1"
                fill="#81C784"
                style="transform-origin: 12px {20 - tween.current * 0.4}px; transform: scale({rightLeafTween.current / 4});"
            ></path>
        {/if}
        ...
        {#if flowerTween.current > 0}
            <!-- Flower center -->
            <circle cx="12" cy={20 - tween.current} r={flowerTween.current} fill="#FFC107"></circle>
