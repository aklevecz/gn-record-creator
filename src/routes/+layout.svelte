<script>
    ...
    let assetsToCache = new Set();

    onMount(() => {
        if (!browser) return;
		console.log("WHATR")
        // Function to detect and collect assets for caching
        const collectAssets = () => {
            // Find all stylesheets
            document.querySelectorAll('link[rel="stylesheet"]').forEach((link) => {
                if (link.href) assetsToCache.add(link.href);
            });

            // Find all scripts
            document.querySelectorAll('script[src]').forEach((script) => {
                if (script.src) assetsToCache.add(script.src);
            });
            ...
            document.querySelectorAll('img[src]').forEach((img) => {
                if (img.src) assetsToCache.add(img.src);
            });

            return Array.from(assetsToCache);
        };
		...
		(async () => {
            ...
            if ('serviceWorker' in navigator) {
                try {
                    ...
                    // When the service worker is active, send it the list of assets to cache
                        ...
                        installingWorker.onstatechange = () => {
                            if (installingWorker.state === 'installed') {
                                if (navigator.serviceWorker.controller) {
                                    ...
                                    // Send the list of assets to cache
                                    const urls = collectAssets();
